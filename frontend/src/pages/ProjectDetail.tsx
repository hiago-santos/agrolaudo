...
const NON_TERMINAL: Project['status'][] = ['DRAFT', 'PENDING_SIGNATURES', 'SIGNED', 'UNDER_BANK_REVIEW'];
...
export function ProjectDetail() {
  ...
  async function submitForReview() {
    ...
    try {
      ...
      toast.error(e instanceof ApiError ? e.message : 'Não foi possível enviar o projeto para o banco.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Summary label="Receita Líquida" value={formatCurrency(project.totalProfit)} highlight />
        <Summary label="Margem Operacional" value={formatPercentage(project.profitMarginPercentage)} />
      </div>

      {(project.status === 'UNDER_BANK_REVIEW' || project.status === 'APPROVED' || project.status === 'REJECTED') && (
        <BankReviewPanel project={project} canReview={canReview} onUpdated={() => void load()} />
      )}
          ...
          <TableBody>
            {project.items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-right">{formatNumber(item.totalProduction)}</TableCell>
                <TableCell className="text-right">{formatCurrency(item.grossRevenue)}</TableCell>
                <TableCell className="text-right">{formatCurrency(item.totalCost)}</TableCell>
                <TableCell className="text-right font-medium text-accent">{formatCurrency(item.netProfit)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      {project.notes && (
        <Card className="p-5">
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-text-tertiary">Observações</p>
...
function Summary({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <Card className="p-4">
      ...
      <p className={highlight ? 'mt-1 text-xl font-semibold text-accent' : 'mt-1 text-xl font-semibold text-text'}>
        {value}
      </p>
    </Card>
  );
